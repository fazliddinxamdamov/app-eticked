...
public class ReturnTariffServiceImpl implements ReturnTariffService {
    ...
    public ApiResult<List<ReturnTariffDto>> getAllByEvent(UUID eventId) {
        List<ReturnTariffDto> returnTariffDtoList = returnTariffRepository.findAllByEventId(eventId).stream().map(returnTariffMapper::toDto).collect(Collectors.toList());
